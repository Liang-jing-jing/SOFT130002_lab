...
function showWindowHref(){
    let str = url.value;
    const a = url.value.indexOf("name=");
    if (a !== -1) {
        let b = str.indexOf("&", a);
        if (b !== -1) {
            url_result.value = str.slice(a + 5, b);//name属性后面还有别的属性
        } else {
            url_result.value = str.slice(a + 5);//name属性作为结尾
        }
    } else {
        url_result.value = "no name argument";//没有name这个属性
    }
}
...
function arrSameStr(){
    ...
    result.value = "The most character is:" + max_value + " times:" + max;
}
